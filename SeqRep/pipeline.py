...
class KMerCountsPipeline(Pipeline):
    ...
    def __init__(self, paths: list[str], K: int, repr_size=2, header_parser='None', quiet=False,
        ...
        sample = self.counter.kmer_counts(self.dataset['seqs'].to_numpy(), quiet=self.quiet)
    ...
    def fit(self, **kwargs):
        ...
        """
        enc = self.preprocess_seqs(self.dataset['seqs'].to_numpy())
        self.model.fit(enc, silent=self.quiet, **kwargs)
